...
/**
 * An FRSLDocument refines a document to support generation of an alternate (XMI) content
 * for use during save in place of its normal textual content.
 */
public class FRSLDocument extends BaseDocument
{
	@Inject
	public FRSLDocument(DocumentTokenSource tokenSource, ITextEditComposer composer) {
		...
		readOnly(new IUnitOfWork<Object, XtextResource>()
		{
			...
			public Object exec(@Nullable XtextResource resource) throws Exception {
				if (resource != null) {
					...
					Status status = new Status(IStatus.INFO, "org.eclipse.sme.frsl", "hanhdd: FRSLDocument@saveAsEcore **** " + asResource + " ****** " + asResource.getContents().size());
					ILog log = Platform.getLog(FRSLDocument.class);
					log.log(status);
					//!MESSAGE hanhdd: FRSLDocument@saveAsEcore **** org.eclipse.sme.frsl.utilities.FRSLASResourceImpl@33c6625c uri='platform:/resource/org.eclipse.ocl.xtext.frsl.examples/test03.frsl.oclas' ****** 0
					...
					if (asResource != null) {
						...
						if (cs2as != null) {
							...
							// ecoreResource.getContents().addAll(resource.getContents());
