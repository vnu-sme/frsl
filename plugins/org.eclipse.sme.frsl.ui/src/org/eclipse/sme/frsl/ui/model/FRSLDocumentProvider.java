 ...
 *******************************************************************************/
package org.eclipse.sme.frsl.ui.model;
...
import org.eclipse.sme.frsl.ui.FRSLUiModule;
import org.eclipse.sme.frsl.ui.FRSLUiPluginHelper;
...
/**
 * FRSLDocumentProvider orchestrates the load and saving of optional XMI content
 * externally while maintaining the serialised human friendly form internally.
 */
public class FRSLDocumentProvider extends BaseCSorASDocumentProvider
{		// FIXME share more code with BaseCSorASDocumentProvider
	private static final Logger log = Logger.getLogger(FRSLDocumentProvider.class);
	...
	public static final String PERSIST_AS_FRSL = "frsl";
	...
	private void diagnoseErrors(Resource resource) throws CoreException {
		...
		if (errors.size() > 0) {
			...
			throw new CoreException(new Status(IStatus.ERROR, FRSLUiModule.PLUGIN_ID, formattedMessage));
		}
	}
	...
	protected void doSaveDocument(IProgressMonitor monitor, Object element, IDocument document, boolean overwrite) throws CoreException {
		...
		if ((element instanceof IFileEditorInput) && (document instanceof FRSLDocument) && !PERSIST_AS_FRSL.equals(saveAs)) {
			StringWriter xmlWriter = new StringWriter();
			try {
				ASResource asResource = ((FRSLDocument) document).getASResource();
				...
				if (uri == null) {
					...
					((FRSLDocument) document).saveAsEcore(xmlWriter, uri, exportDelegateURIMap.get(document));
				}
				else if (PERSIST_IN_ECORE.equals(saveAs)) {
					((FRSLDocument) document).saveInEcore(xmlWriter, uri, exportDelegateURIMap.get(document));
				}
					...
					try {
						...
						((FRSLDocument) document).saveAsPivot(xmlWriter);		// FIXME localize/avoid uri tweak
					}
					finally {
					...
					((FRSLDocument) document).saveAsUML(xmlWriter, uri);
				}
				else {
				...
				FRSLUiPluginHelper helper = FRSLUiPluginHelper.INSTANCE;
	...
	protected String getCScontentType() {
		return FrslCSPackage.eCONTENT_TYPE;
	}
	...
	protected String getPluginId() {
		return FRSLUiModule.PLUGIN_ID;
	}

	@Override
	protected boolean isText(String loadedAs) {
		return PERSIST_AS_FRSL.equals(loadedAs) || super.isText(loadedAs);
	}
	...
	protected void setDocumentContent(IDocument document, InputStream inputStream, String encoding) throws CoreException {		
		...
		try {
			...
			String persistAs = PERSIST_AS_FRSL;
			
			ILog log = Platform.getLog(FRSLDocumentProvider.class);			
			IStatus logMsg;
			...
			if (isXML) {	
				...
				logMsg = new Status(IStatus.INFO, "org.eclipse.sme.frsl", "hanhdd: FRSLDocumentProvider$setDocumentContent" 
						);
				log.log(logMsg);
				...
				if (contents.size() > 0) {
					...
					if (xmiRoot instanceof EPackage) {
						...
						asResource = (ASResource) pivotModel.eResource();
						persistAs = PERSIST_AS_FRSL;		// FIXME
					}
...
//		FRSLUtil.removeFrslModelResource( this.getEnvironmentFactory().getMetamodelManager() );
//		super.disconnected();
//	}
}
